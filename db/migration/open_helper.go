...
import (
	...
	. "github.com/concourse/migrations/migration"
)
...
func (self *OpenHelper) CurrentVersion() (int, error) {
	...
	return NewMigrator(db, self.lockFactory, self.strategy, &bindataSource{}).CurrentVersion()
}
...
func (self *OpenHelper) SupportedVersion() (int, error) {
	...
	return NewMigrator(db, self.lockFactory, self.strategy, &bindataSource{}).SupportedVersion()
}
...
func (self *OpenHelper) Open() (*sql.DB, error) {
	...
	if err := NewMigrator(db, self.lockFactory, self.strategy, &bindataSource{}).Up(); err != nil {
		_ = db.Close()
		return nil, err
	}
...
func (self *OpenHelper) OpenAtVersion(version int) (*sql.DB, error) {
	...
	if err := NewMigrator(db, self.lockFactory, self.strategy, &bindataSource{}).Migrate(version); err != nil {
		_ = db.Close()
		return nil, err
	}
...
func (self *OpenHelper) MigrateToVersion(version int) error {
	...
	defer db.Close()
	m := NewMigrator(db, self.lockFactory, self.strategy, &bindataSource{})
